"""Generate figures for the evaluation report.

Read-only: consumes analysis/data/*.csv (+ core_metrics.pickle) produced by the
other analysis scripts. Writes PNGs to analysis/reports/figures/.

Figures:
  1. moral_count_by_strategy.png   - moral words preserved, with oracle ceiling
  2. moral_div_by_strategy.png     - moral-frame divergence (lower better)
  3. refusal_rate_by_strategy.png  - refusal % per strategy (the headline)
  4. response_breakdown_stacked.png- ok/refusal/format_miss/no_summary per strategy
  5. zeroshot_vs_fewshot.png       - grouped comparison on both moral metrics
  6. moral_count_by_dataset.png    - per-dataset breakdown (heatmap-style)
...
DATA = os.path.join(ROOT, 'analysis', 'llama')
FIGS = os.path.join(ROOT, 'analysis', 'llama', 'figures')
...
# Consistent color coding: grey=zero-shot baselines, gold=oracle, teal=few-shot (ours)
def bar_colors(strats):
    out = []
    for s in strats:
        if s == 'oracle':
            out.append('#d4a017')      # gold ceiling
        elif s in FEWSHOT:
            out.append('#2a9d8f')      # teal = our contribution
        else:
            out.append('#8d99ae')      # grey = zero-shot baselines
    return out
...
def load_summary():
    return pd.read_csv(os.path.join(DATA, 'strategy_summary.csv'))


def load_refusal():
    return pd.read_csv(os.path.join(DATA, 'refusal_by_strategy.csv'))
...
def fig_moral_count(summary):
    ...
    bars = ax.bar(range(len(STRATEGIES)), vals.values, color=bar_colors(STRATEGIES))
    # highlight best non-oracle
    best = vals.drop('oracle').idxmax()
    ax.axhline(vals['vanilla'], color='#8d99ae', ls='--', lw=1, alpha=.7,
               label=f"vanilla baseline ({vals['vanilla']:.2f})")
    ax.axhline(vals['oracle'], color='#d4a017', ls='--', lw=1, alpha=.7,
               label=f"oracle ceiling ({vals['oracle']:.2f})")
    ...
    ax.set_title('Moral-word preservation by strategy  (higher = better)\n'
                 f'best non-oracle: {best} ({vals[best]:.2f})', fontsize=12)
    ax.legend(handles=LEGEND + [
        plt.Line2D([], [], color='#8d99ae', ls='--', label=f"vanilla ({vals['vanilla']:.2f})"),
        plt.Line2D([], [], color='#d4a017', ls='--', label=f"oracle ({vals['oracle']:.2f})"),
...
def fig_moral_div(summary):
    ...
    ax.set_title('Moral-frame divergence by strategy  (LOWER = better)\n'
...
def fig_refusal_rate(refusal):
    ...
    ax.set_title('Safety refusals by strategy  (lower = better)\n'
                 'few-shot conditioning nearly eliminates refusals', fontsize=12)
    ax.legend(handles=LEGEND, fontsize=8, loc='upper right')
...
def fig_response_breakdown(refusal):
    ...
    for cat in cats:
        vals = r[cat].values
        ax.bar(range(len(STRATEGIES)), vals, bottom=bottom,
               color=colors[cat], label=cat)
        bottom += vals
    ax.set_xticks(range(len(STRATEGIES)))
    ax.set_xticklabels([LABELS[s] for s in STRATEGIES], fontsize=9)
    ax.set_ylabel('Responses (of 400)')
    ax.set_title('Response outcome breakdown by strategy', fontsize=12)
...
def fig_zeroshot_vs_fewshot(summary, refusal):
    ...
    for ax, (name, fn) in zip(axes, metrics.items()):
        ...
        bars = ax.bar(['Zero-shot', 'Few-shot'], vals,
                      color=['#8d99ae', '#2a9d8f'])
    ...
    fig.suptitle('Zero-shot vs Few-shot (our contribution)', fontsize=13)
...
def fig_moral_count_by_dataset(summary):
    ...
    ax.set_yticklabels([s.replace('\n', ' ') for s in
                        [LABELS[x] for x in STRATEGIES]], fontsize=9)
    for i in range(len(STRATEGIES)):
        for j in range(len(datasets)):
            ax.text(j, i, f'{mat[i, j]:.1f}', ha='center', va='center',
                    color='black' if mat[i, j] < mat.max() * 0.6 else 'white',
                    fontsize=9)
    ax.set_title('Moral words preserved by strategy × dataset', fontsize=12)
...
def fig_paired_contrasts(paired):
    """One PNG per contrast pair, both metrics side by side."""
    ...
    for strat_a, strat_b in pairs:
        ...
        fig.suptitle(f'Llama-3.1-8B: {strat_a}  vs  {strat_b}\n'
...
def main():
    ...
    if os.path.isfile(paired_path):
        saved = fig_paired_contrasts(pd.read_csv(paired_path))
        print(f'Wrote {len(saved)} paired-contrast figures to', FIGS)
